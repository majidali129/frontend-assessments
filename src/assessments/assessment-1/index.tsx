...
import { TaskMangerFilters } from "./components/filters";
import { TaskManagerHeader } from "./components/header";
import { TaskManagerStats } from "./components/stats";
import { TaskList } from "./components/task-list";
import { useEffect, useState } from "react";
import type { Filter, Task, TaskStatus } from "./types";
import { taskList } from "./data";
import { TaskManagerSearch } from "./components/search";
import { TaskManagerFilterTabs } from "./components/filter-tabs";
...
export const TaskManger = () => {
  ...
  const handleToggleStatus = (targetId: string, status: TaskStatus) => {
    const updatedTasks = tasks.map((task) =>
      task.id === targetId ? { ...task, status } : task,
    );
